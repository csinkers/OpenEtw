...
let buildFiles arguments provider =
    ...
    | CppSelfDescribing -> 
        ...
        let implementationOptions = 
            {
                etwGenComment = arguments.etwGenComment
    ...
    | NodeJs ->
        let implementationOptions = 
            {
                etwGenComment = Util.buildEtwGenComment arguments.inputFile
...
let parseCommandLine argv =
    ...
    | ParserResult.Success (results, _, _) ->
...

        {
            ...
            etwGenComment = Util.buildEtwGenComment fullPath
...
let generateEtwProvider arguments =
    ...
    | Success provider ->
        let safetyCheck filename =
            if (not (System.IO.File.Exists filename)) then None else
            let etwGenComment = Util.extractEtwGenComment filename
            if etwGenComment = None then
        ...
        implementation |> Seq.iter (fun (filename, content) ->
            let existingHeader = safetyCheck filename // Ensure we only overwrite auto-generated files.
            if (existingHeader = Some arguments.etwGenComment) then () else // Do nothing if input and EtwGen version haven't changed
