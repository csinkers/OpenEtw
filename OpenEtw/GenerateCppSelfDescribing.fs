...
let buildCpp (provider : EtwProvider) (options : CppSelfDescribingOptions) =
    ...
    [
        (if options.precompiledHeader.IsSome then sprintf "#include \"%s\"" options.precompiledHeader.Value else "")
        (sprintf """// -----------------------------------------------------
...
#define ETW_ANSI_PARAM(n, p) const char *arg##n = (p);\
    ...
    EventDataDescCreate(&eventData[n],\
            ...
            (arg##n != nullptr) ? (arg##n##Len > 64000 ? 64000 : arg##n##Len) : (ULONG)sizeof("NULL"));
                    ...
                    sprintf """void %s::%s()
{
    ...
    EventWrite(ProviderHandle, &Event_%s, 0, nullptr);
}
"""                     provider.className
//                        provider.prefix
                        e.cppName
                    ...
                    sprintf """void %s::%s()
{
    ETW_ENABLED_CHECK(%d)
    ETW_DEBUG("ETW_DEBUG: Emitting event '%s'\n");
    EventWriteEx(ProviderHandle, &Event_%s, 0, 0, %s, %s, 0, nullptr);
}
"""                     provider.className
//                        provider.prefix
                        e.cppName
...
//                        (match e.supplementaryCode with |Some x -> x.Replace("\n", System.Environment.NewLine) |None -> "")
                        e.cppName
                        e.symbol
                        (activityId |> Option.map (fun p -> p.name) |?? "nullptr")
                        (relatedActivityId |> Option.map (fun p -> p.name) |?? "nullptr")
